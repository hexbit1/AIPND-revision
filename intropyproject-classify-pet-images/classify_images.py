...
def classify_images(images_dir, results_dic, model):
    ...
    for k,v in results_dic.items():
        test_image = "{}{}".format(images_dir,k)
        image_classification = classifier(test_image, model).lower().strip()
        val = [clabel.strip() for clabel in image_classification.split(",")]
        results_dic[k].append(image_classification)
        results_dic[k].append(int(v[0] in val))